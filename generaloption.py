...
# supported in optparse of python v2.4
...
def set_columns(cols=None):
    ...
    if cols is None:
        ...
        if os.path.exists(stty):
            try:
                cols = int(os.popen('%s size 2>/dev/null' %
                                    stty).read().strip().split(' ')[1])
...
def check_str_list_tuple(option, opt, value):  # pylint: disable=unused-argument
    ...
    if klass is None:
        err = _gettext(
            "check_strlist_strtuple: unsupported type %s" % option.type)
...
class ExtOption(CompleterOption):
    ...
    ACTIONS = Option.ACTIONS + EXTOPTION_EXTRA_OPTIONS + \
        EXTOPTION_STORE_OR + EXTOPTION_LOG + EXTOPTION_HELP
    STORE_ACTIONS = Option.STORE_ACTIONS + \
        EXTOPTION_EXTRA_OPTIONS + EXTOPTION_LOG + ('store_or_None',)
    TYPED_ACTIONS = Option.TYPED_ACTIONS + \
        EXTOPTION_EXTRA_OPTIONS + EXTOPTION_STORE_OR
    ALWAYS_TYPED_ACTIONS = Option.ALWAYS_TYPED_ACTIONS + EXTOPTION_EXTRA_OPTIONS

    TYPE_STRLIST = ['%s%s' % (name, klass) for klass in [
        'list', 'tuple'] for name in ['str', 'path']]
    TYPE_CHECKER = dict([(x, check_str_list_tuple)
                         for x in TYPE_STRLIST] + list(Option.TYPE_CHECKER.items()))
        ...
        if self.action == 'extend':
            ...
            if self.store_or in self.EXTOPTION_STORE_OR:
                ...
                self.log.raiseException(
                    "_set_attrs: unknown store_or %s" % self.store_or, exception=ValueError)

    def process(self, opt, value, values, parser):
        """Handle option-as-value issues before actually processing option."""
        print( """Handle option-as-value issues before actually processing option.""")
    ...
    def take_action(self, action, dest, opt, value, values, parser):
        ...
        if dest and getattr(parser._long_opt.get('--' + dest, ''), 'store_or', '') == 'help':
            ...
            if fn is None:
                self.log.raiseException(
                    "Unsupported output format for help: %s" % value.help, exception=ValueError)
            else:
                fn()
            parser.exit()
            ...
            if orig_action in self.EXTOPTION_LOG and action == 'store_true':
                ...
                logstate = ", ".join(["(%s, %s)" % (n, l)
                                      for n, l in getDetailsLogLevels()])
                self.log.debug(
                    "changing loglevel to %s, current state: %s", newloglevel, logstate)
                setLogLevel(newloglevel)
                self.log.debug(
                    "changed loglevel to %s, previous state: %s", newloglevel, logstate)
            ...
            if action in ("add", "add_first", "add_flex",):
                ...
                self.log.raiseException(
                    msg % (action, self.EXTOPTION_EXTRA_OPTIONS))
...
class ExtOptionGroup(OptionGroup):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if section_name in self.RESERVED_SECTIONS:
            self.log.raiseException(
                'Cannot use reserved name %s for section name.' % section_name)
...
class ExtOptionParser(OptionParser):
    ...
    # exact match for option name (without the '-') as value
    ALLOW_OPTION_NAME_AS_VALUE = False
    ALLOW_OPTION_AS_VALUE = False  # exact match for option as value
    ALLOW_DASH_AS_VALUE = False  # any value starting with a '-'
    # value with similarity score from difflib.get_close_matches
    ALLOW_TYPO_AS_VALUE = True
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.error_env_option_method = kwargs.pop(
            'error_env_option_method', self.log.error)
        ...
        if not hasattr(self.formatter, 'format_epilog'):
            setattr(self.formatter, 'format_epilog',
                    self.formatter.format_description)
        ...
        if hasattr(self.option_class, 'ENABLE') and hasattr(self.option_class, 'DISABLE'):
            ...
            self.epilog.append(epilogtxt %
                               {'disable': self.option_class.DISABLE})
    ...
    def set_description_docstring(self):
        ...
        try:
            ...
            self.log.debug(
                "set_description_docstring: no docstring found in latest stack globals")
            docstr = None
    ...
    def check_help(self, fh):
        ...
        if hasattr(self.option_class, 'ENABLE') and hasattr(self.option_class, 'DISABLE'):
            ...
            for opt in self._get_all_options():
                ...
                opt._long_opts = [
                    x for x in opt._long_opts if not _is_enable_disable(x)]
        return fh
    ...
    def print_rsthelp(self, fh=None):
        ...
        if self.usage:
            title = "Usage"
            result.extend([title, '-' * len(title), '', '``%s``' %
                           self.get_usage().replace("Usage: ", '').strip(), ''])
        if self.description:
            title = "Description"
        ...
        for title, opts in all_opts:
            ...
            for opt in opts:
                if opt.help is not nohelp:
                    values.append(
                        ['``%s``' % formatter.option_strings[opt], formatter.expand_default(opt)])
    ...
    def _add_help_option(self):
        ...
        self.add_option("-%s" % self.shorthelp[0],
                        # *self.shorthelp[1:], syntax error in Python 2.4
                        self.shorthelp[1],
                        action="shorthelp",
                        help=_gettext("show short help message and exit"))
        self.add_option("-%s" % self.longhelp[0],
                        # *self.longhelp[1:], syntax error in Python 2.4
                        self.longhelp[1],
                        action="help",
                        type="choice",
    ...
    def _get_args(self, args):
        ...
        # prepend the environment options as longopts
        # return self.environment_arguments + self.commandline_arguments
    ...
    def get_env_options(self):
        ...
        print(("get_env_options",self.environment_arguments))
        ...
        candidates = dict([(k, v) for k, v in os.environ.items()
                           if k.startswith("%s_" % self.envvar_prefix)])
        ...
        for opt in self._get_all_options():
            ...
            for lo in opt._long_opts:
                if len(lo) == 0:
                    continue
                env_opt_name = "%s_%s" % (
                    self.envvar_prefix, lo.lstrip('-').replace('-', '_').upper())
                ...
                if val is not None:
                    ...
                    self.log.debug(
                        "Environment variable %s is not set" % env_opt_name)
        ...
        if candidates:
            ...
            logmethod(msg, len(candidates), self.envvar_prefix,
...
class GeneralOption(object):
    ...
    CONFIGFILES_IGNORE = []
    # sectionname that contains the non-grouped/non-prefixed options
    CONFIGFILES_MAIN_SECTION = 'MAIN'
    ...
    def __init__(self, **kwargs):
        ...
        self.no_system_exit = kwargs.pop(
            'go_nosystemexit', None)  # unit test option
        self.use_configfiles = kwargs.pop(
            'go_useconfigfiles', self.CONFIGFILES_USE)  # use or ignore config files
        self.configfiles = kwargs.pop(
            'go_configfiles', self.CONFIGFILES_INIT[:])  # configfiles to parse
        # initial environment for configfiles to parse
        configfiles_initenv = kwargs.pop('go_configfiles_initenv', None)
        # name of logger is same as envvar prefix
        prefixloggername = kwargs.pop('go_prefixloggername', False)
        # Set the main options before the default ones
        mainbeforedefault = kwargs.pop('go_mainbeforedefault', False)
        # Pass these options to the autocomplete call
        autocompleter = kwargs.pop('go_autocompleter', {})
        ...
        if prefixloggername:
            ...
            if prefix is not None and len(prefix) > 0:
                # . indicate hierarchy in logging land
                loggername = prefix.replace('.', '_')
    ...
    def _make_debug_options(self):
        ...
        self.log.debug("Add debug and logging options descr %s opts %s (no prefix)" % (
            descr, self._logopts))
    ...
    def _set_default_loglevel(self):
        ...
        loglevel_set = sum([getattr(self.options, name, False)
                            for name in self._logopts.keys()])
    ...
    def _make_configfiles_options(self):
        ...
        self.log.debug(
            "Add configfiles options descr %s opts %s (no prefix)" % (descr, opts))
        self.add_group_parser(opts, descr, prefix=None,
                              section_name=ExtOptionGroup.NO_SECTION)
    ...
    def main_options(self):
        ...
        if hasattr(self, 'make_init'):
            self.log.debug(
                'main_options: make_init is deprecated. Rename function to main_options.')
            ...
            if len(names) == 0:
                self.log.error(
                    "main_options: no options functions implemented")
            else:
                for name in names:
    ...
    def add_group_parser(self, opt_dict, description, prefix=None, otherdefaults=None, section_name=None):
        ...
        self.log.debug("add_group_parser: passed prefix %s section_name %s" % (
            prefix, section_name))
        ...
        self.log.debug("add_group_parser: set prefix %s section_name %s" % (
            prefix, section_name))
        ...
        opt_grp = ExtOptionGroup(
            self.parser, short_description, long_description, section_name=section_name)
        ...
        for key in keys:
            ...
            if typ in ExtOption.TYPE_STRLIST:
                ...
                extra_help.append("type %s-separated %s" %
                                  (helpsep, klass.__name__))
            ...
            opt_name, opt_dest = self.make_options_option_name_and_destination(
                prefix, key)

            args = ["--%s" % opt_name]

            # this has to match PROCESSED_OPTIONS_PROPERTIES
            self.processed_options[opt_dest] = [
                typ, default, action, opt_name, prefix, section_name]  # add longopt
            if len(self.processed_options[opt_dest]) != len(self.PROCESSED_OPTIONS_PROPERTIES):
                self.log.raiseException(
                    "PROCESSED_OPTIONS_PROPERTIES length mismatch")
            ...
            if len(details) >= 5:
                for extra_detail in details[4:]:
                    if isinstance(extra_detail, (list, tuple,)):
                        # choices
                        nameds['choices'] = ["%s" %
                                             x for x in extra_detail]  # force to strings
                        ...
                        self.log.raiseException(
                            "add_group_parser: unknown extra detail %s" % extra_detail)
            ...
            if hasattr(self.parser.option_class, 'ENABLE') and hasattr(self.parser.option_class, 'DISABLE'):
                if action in self.parser.option_class.BOOLEAN_ACTIONS:
                    args.append("--%s-%s" %
                                (self.parser.option_class.ENABLE, opt_name))
                    args.append("--%s-%s" %
                                (self.parser.option_class.DISABLE, opt_name))
        ...
        if section_name not in prefix_section_names:
            ...
            self.log.debug("Added prefix %s to list of sectionnames for %s" % (
                prefix, section_name))
    ...
    def autocomplete(self):
        ...
        if self.autocompleter is None:
            self.log.debug(
                'self.autocompleter is None, disabling autocompleter')
        else:
            self.log.debug('setting autocomplete with args %s' %
                           self.autocompleter)
    ...
    def parseoptions(self, options_list=None):
        ...
        self.log.debug("parseoptions: options from environment %s" %
                       (self.parser.environment_arguments))
        self.log.debug("parseoptions: options from commandline %s" %
                       (self.parser.commandline_arguments))
    ...
    def parseconfigfiles(self):
        ...
        if not self.use_configfiles:
            self.log.debug(
                'parseconfigfiles: use_configfiles False, skipping configfiles')
            return
        ...
        self.log.debug(
            "parseconfigfiles: configfiles initially set %s" % self.configfiles)

        option_configfiles = self.options.__dict__.get(
            'configfiles', [])  # empty list, will win so no defaults
        option_ignoreconfigfiles = self.options.__dict__.get(
            'ignoreconfigfiles', self.CONFIGFILES_IGNORE)

        self.log.debug(
            "parseconfigfiles: configfiles set through commandline %s" % option_configfiles)
        self.log.debug(
            "parseconfigfiles: ignoreconfigfiles set through commandline %s" % option_ignoreconfigfiles)
        ...
        for fn in self.configfiles:
            if not os.path.isfile(fn):
                if self.CONFIGFILES_RAISE_MISSING:
                    self.log.raiseException(
                        "parseconfigfiles: configfile %s not found." % fn)
                else:
                    self.log.debug(
                        "parseconfigfiles: configfile %s not found, will be skipped" % fn)

            if fn in option_ignoreconfigfiles:
                self.log.debug(
                    "parseconfigfiles: configfile %s will be ignored", fn)
            else:
        ...
        self.log.debug(
            "parseconfigfiles: following files were parsed %s" % parsed_files)
        ...
        for section in cfg_sections:
            if section not in self.config_prefix_sectionnames_map.values():
                self.log.warning(
                    "parseconfigfiles: found section %s, won't be parsed" % section)
                continue

        # add any non-option related configfile data to configfile_remainder dict
        cfg_sections_flat = [
            name for section_names in cfg_sections for name in section_names]
        for section in self.configfile_parser.sections():
            if section not in cfg_sections_flat:
                self.log.debug(
                    "parseconfigfiles: found section %s, adding to remainder" % section)
        ...
        for prefix, section_names in self.config_prefix_sectionnames_map.items():
            for section in section_names:
                ...
                if not self.configfile_parser.has_section(section):
                    self.log.debug(
                        'parseconfigfiles: no section %s' % str(section))
                    continue
                elif section == ExtOptionGroup.NO_SECTION:
                    self.log.debug(
                        'parseconfigfiles: ignoring NO_SECTION %s' % str(section))
                    continue
                elif section.lower() == 'default':
                    self.log.debug(
                        'parseconfigfiles: ignoring default section %s' % section)
                    continue

                for opt, val in self.configfile_parser.items(section):
                    self.log.debug(
                        'parseconfigfiles: section %s option %s val %s' % (section, opt, val))

                    opt_name, opt_dest = self.make_options_option_name_and_destination(
                        prefix, opt)
                    actual_option = self.parser.get_option_by_long_name(
                        opt_name)
                    if actual_option is None:
                        # don't fail on DEFAULT UPPERCASE options in case-sensitive mode.
                        in_def = self.configfile_parser.has_option(
                            self.DEFAULTSECT, opt)
                    ...
                    log_action_taken = getattr(
                        self.options, '_logaction_taken', False)
                    if is_log_action and log_action_taken:
                        ...
                        try:
                            newval = self.configfile_parser.getboolean(
                                section, opt)
        ...
        # reparse
        self.log.debug(
            'parseconfigfiles: going to parse options through cmdline %s' % configfile_cmdline)
        try:
            ...
            (parsed_configfile_options, parsed_configfile_args) = self.parser.parse_args(
                configfile_cmdline)
        ...
        self.log.debug("parseconfigfiles: options from configfile %s" %
                       (self.parser.commandline_arguments))

        if len(parsed_configfile_args) > 0:
            self.log.raiseException(
                'parseconfigfiles: not all options were parsed: %s' % parsed_configfile_args)

        for opt_dest in configfile_cmdline_dest:
            try:
                configfile_values[opt_dest] = getattr(
                    parsed_configfile_options, opt_dest)
            except AttributeError:
                self.log.raiseException('parseconfigfiles: failed to retrieve dest %s from parsed_configfile_options' %
                                        opt_dest)

        self.log.debug(
            'parseconfigfiles: parsed values from configfiles: %s' % configfile_values)

        for opt_dest, val in configfile_values.items():
            set_opt = False
            if not hasattr(self.options, opt_dest):
                self.log.debug(
                    'parseconfigfiles: adding new option %s with value %s' % (opt_dest, val))
                set_opt = True
            else:
                if hasattr(self.options, '_action_taken') and self.options._action_taken.get(opt_dest, None):
                    # value set through take_action. do not modify by configfile
                    self.log.debug(
                        'parseconfigfiles: option %s already found in _action_taken' % (opt_dest))
                else:
    ...
    def _get_options_by_property(self, prop_type, prop_value):
        ...
        for key in [dest for dest, props in self.processed_options.items() if props[prop_idx] == prop_value]:
            # None? isn't there always a default
            options[key] = getattr(self.options, key, None)
    ...
    def dict_by_prefix(self, merge_empty_prefix=False):
        ...
        if merge_empty_prefix and '' in subdict:
            ...
            for opt, val in subdict[''].items():
                if opt in subdict:
                    self.log.error(
                        "dict_by_prefix: non-prefixed option %s conflicts with prefix of same name." % opt)
                else:
                    subdict[opt] = val
    ...
    def generate_cmd_line(self, ignore=None, add_default=None):
        ...
        for opt_dest in opt_dests:
            ...
            typ = self.processed_options[opt_dest][self.PROCESSED_OPTIONS_PROPERTIES.index(
                'type')]
            default = self.processed_options[opt_dest][self.PROCESSED_OPTIONS_PROPERTIES.index(
                'default')]
            action = self.processed_options[opt_dest][self.PROCESSED_OPTIONS_PROPERTIES.index(
                'action')]
            opt_name = self.processed_options[opt_dest][self.PROCESSED_OPTIONS_PROPERTIES.index(
                'opt_name')]
            ...
            if action in ExtOption.EXTOPTION_STORE_OR:
                if opt_value == default:
                    ...
                    if typ in ExtOption.TYPE_STRLIST:
                        ...
                        args.append("--%s=%s" %
                                    (opt_name, shell_quote(sep.join(opt_value))))
                    else:
                        args.append("--%s=%s" %
                                    (opt_name, shell_quote(opt_value)))
                    ...
                    if opt_value == default and ((action in ('store_true',) +
                        ...
                        if hasattr(self.parser.option_class, 'ENABLE') and \
                           hasattr(self.parser.option_class, 'DISABLE'):
                            args.append(
                                "--%s-%s" % (self.parser.option_class.DISABLE, opt_name))
                        else:
                    ...
                    if action == 'add_flex' and default:
                        for ind, elem in enumerate(opt_value):
                            if elem == default[0] and opt_value[ind:ind+len(default)] == default:
                                empty = get_empty_add_flex(
                                    opt_value, self=self)
                                # TODO: this will only work for tuples and lists
                                opt_value = opt_value[:ind] + type(opt_value)(
                                    [empty]) + opt_value[ind+len(default):]
                ...
                self.log.debug("generate_cmd_line adding %s value %s. append action, return as multiple args" %
                               (opt_name, opt_value))
                args.extend(["--%s=%s" % (opt_name, shell_quote(v))
                             for v in opt_value])
            elif action in ("regex",):
                self.log.debug("generate_cmd_line adding %s regex pattern %s" % (
                    opt_name, opt_value.pattern))
                args.append("--%s=%s" %
                            (opt_name, shell_quote(opt_value.pattern)))
            else:
                self.log.debug("generate_cmd_line adding %s value %s" %
                               (opt_name, opt_value))
...
class SimpleOption(GeneralOption):
    ...
    def __init__(self, go_dict=None, descr=None, short_groupdescr=None, long_groupdescr=None, config_files=None):
        ...
        if descr is not None:
            ...
            self.log.deprecated(
                'SimpleOption descr argument', '2.5.0', '3.0.0')
