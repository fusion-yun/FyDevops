...
RUN --mount=type=cache,uid=1000,id=fy_eb_sources,target=/packages/sources,sharing=shared \
    source /etc/profile.d/lmod.bash  && module load EasyBuild/4.0.1 &&\
    eb foss-2019b.eb --fetch --info -lr && \
    eb foss-2017a.eb --fetch --info -lr 

    
RUN --mount=type=cache,uid=1000,id=fy_eb_sources,target=/packages/sources,sharing=shared \ 
    --mount=type=tmpfs,target=/packages/build \
    source /etc/profile.d/lmod.bash  && module load EasyBuild/4.0.1  &&\
    ls -lh /packages/ &&\
    ls -lh /packages/sources &&\
    echo ${FYDEV_USER_ID}
