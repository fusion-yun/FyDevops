...
ARG CACHE=${CACHE:-fy_pkgs}

LABEL Description   "fyBox: lmod ${FY_LMOD_VERSION} + EasyBuild ${FY_EB_VERSION}, PKG_DIR=${PKG_DIR} FYDEV_USER=${FYDEV_USER}:1000 "
LABEL Name          "fyBox"
...
# Install lmod
RUN --mount=type=cache,uid=1000,id=fy_pkg,target=/packages,sharing=shared \
...
# -------------------------------------------------------------------------------
# 
RUN --mount=type=cache,uid=1000,id=fy_pkg,target=/packages/cache,sharing=shared \
